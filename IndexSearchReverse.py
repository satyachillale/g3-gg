...
def build_index(args):
    if args.index == 'g3':
        ...
        dataloader = DataLoader(dataset, batch_size=256, shuffle=False, num_workers=6, pin_memory=True, prefetch_factor=3)
        model.eval()
        t= tqdm(dataloader)
        ...
        faiss.write_index(index_flat, f'./index/{args.index}_reverse.index')
...
def search_index(args, index, topk):
    ...
    if args.dataset == 'im2gps3k':
        if args.index == 'g3':
            ...
            dataloader = DataLoader(dataset, batch_size=256, shuffle=False, num_workers=6, pin_memory=True, prefetch_factor=5)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(f'./index/{args.index}_reverse.index'):
        build_index(args)
    else:
        # gpu_index_flat = faiss.index_cpu_to_gpu(res, 0, index)
        if not os.path.exists(f'./index/I_{args.index}_{args.dataset}_reverse.npy'):
            index = faiss.read_index(f'./index/{args.index}_reverse.index')
            print('read index success')
            D,I = search_index(args, index, 20)
            np.save(f'./index/D_{args.index}_{args.dataset}_reverse.npy', D)
            np.save(f'./index/I_{args.index}_{args.dataset}_reverse.npy', I)
        else:
            D = np.load(f'./index/D_{args.index}_{args.dataset}_reverse.npy')
            I = np.load(f'./index/I_{args.index}_{args.dataset}_reverse.npy')
        #evaluate(args, I)
